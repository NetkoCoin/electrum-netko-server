...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        self.netkod_url = 'http://%s:%s@%s:%s/' % (
            config.get('netkod', 'netkod_user'),
            config.get('netkod', 'netkod_password'),
            config.get('netkod', 'netkod_host'),
            config.get('netkod', 'netkod_port'))
    ...
    def do_catch_up(self):
        ...
        self.header = self.block2header(self.netkod('getblock', [self.storage.last_hash]))
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("netkod is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def netkod(self, method, params=[]):
        postdata = dumps({"method": method, 'params': params, 'id': 'jsonrpc'})
        while True:
            try:
                respdata = urllib.urlopen(self.netkod_url, postdata).read()
                break
            except:
                print_log("cannot reach netkod...")
                self.shared.pause()
                time.sleep(10)
    ...
    def get_header(self, height):
        block_hash = self.netkod('getblockhash', [height])
        b = self.netkod('getblock', [block_hash])
    ...
    def get_mempool_transaction(self, txid):
        try:
            raw_tx = self.netkod('getrawtransaction', [txid, 0])
        except:
            return None
    ...
    def import_block(self, block, block_hash, block_height, sync, revert=False):
        ...
        # add undo info
        if not revert:
            self.storage.write_undo_info(block_height, self.netkod_height, undo_info)
    ...
    def process(self, request, cache_only=False):
        ...
        if method == 'blockchain.numblocks.subscribe':
            ...
            try:
                txo = self.netkod('sendrawtransaction', params)
            ...
            tx_hash = params[0]
            result = self.netkod('getrawtransaction', [tx_hash, 0])

        elif method == 'blockchain.estimatefee':
            num = int(params[0])
            result = self.netkod('estimatefee', [num])
    ...
    def getfullblock(self, block_hash):
        block = self.netkod('getblock', [block_hash])
        ...
        try:
            ...
            logger.error("netkod error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run netkod with txindex=1; use -reindex if needed.")
    ...
    def catch_up(self, sync=True):
        ...
        while not self.shared.stopped():
            ...
            # are we done yet?
            info = self.netkod('getinfo')
            self.netkod_height = info.get('blocks')
            netkod_block_hash = self.netkod('getblockhash', [self.netkod_height])
            if self.storage.last_hash == netkod_block_hash:
            ...
            self.up_to_date = False
            try:
                next_block_hash = self.netkod('getblockhash', [self.storage.height + 1])
    ...
    def memorypool_update(self):
        mempool_hashes = set(self.netkod('getrawmempool'))
