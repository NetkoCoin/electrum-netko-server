...
if os.path.dirname(os.path.realpath(__file__)) == os.getcwd():
    imp.load_module('electrumnetkoserver', *imp.find_module('src'))

from electrumnetkoserver import storage, networks, utils
from electrumnetkoserver.processor import Dispatcher, print_log
from electrumnetkoserver.server_processor import ServerProcessor
from electrumnetkoserver.blockchain_processor import BlockchainProcessor
from electrumnetkoserver.stratum_tcp import TcpServer
from electrumnetkoserver.stratum_http import HttpServer
...
def create_config(filename=None):
    ...
    config.set('server', 'banner', 'Welcome to netko Electrum!')
    config.set('server', 'banner_file', '/etc/electrum-netko.banner')
    ...
    config.set('server', 'coin', 'netko')
    config.set('server', 'logfile', '/var/log/electrum-netko.log')
    config.set('server', 'donation_address', '')
    config.set('server', 'max_subscriptions', '10000')

    config.add_section('leveldb')
    config.set('leveldb', 'path', '/dev/shm/electrum-netko_db')
    ...
    config.set('network', 'type', 'netko_main')

    # try to find the config file in the default paths
    if not filename:
        for path in ('/etc/', ''):
            filename = path + 'electrum-netko.conf'
